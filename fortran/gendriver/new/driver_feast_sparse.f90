...
!open(10,file=trim(name)//'_B.isa',status='replace')
!     write(10,*) n
!      do i=1,n+1
!        write(10,*) isb(i)
!      end do
!     close(10)
!    open(10,file=trim(name)//'_B.jsa',status='replace')
!     write(10,*) nnzb
!      do i=1,nnzb
!         write(10,*) jsb(i)
!      end do
!      close(10)
!  
!     open(10,file=trim(name)//'_B.sa',status='replace')
!      write(10,*) nnzb
!      do i=1,nnzb
!         write(10,*) dsb(i)
!      end do
...
if (fpm(11)==0) then ! pardiso
        call dfeast_scsrgv(UPLO,N,dsa,isa,jsa,dsb,isb,jsb,fpm,depsout,loop,dEmin,dEmax,M0,dE,dX,M,dres,info)
else ! iterative

        call dfeast_scsrgvit(UPLO,N,dsa,isa,jsa,dsb,isb,jsb,fpm,depsout,loop,dEmin,dEmax,M0,dE,dX,M,dres,info)
endif
     ...
     elseif  ((PRE=='d').and.(EG=='e')) then 
        ...
        allocate(dX(1:n,1:M0))
   call dfeast_scsrev(UPLO,N,dsa,isa,jsa,fpm,depsout,loop,dEmin,dEmax,M0,dE,dX,M,dres,info) ! direct solver
   else !1 gmres, 3 minres
        if (n/=ncol) then
print *,'===>Solving the singular value problem AA^T or A^TA -svd between',demin,demax
dEmin=dEmin**2
dEmax=dEmax**2
endif
        allocate(dX(1:min(n,ncol),1:M0))
        call dfeast_scsrevit(UPLO,N,ncol,dsa,isa,jsa,fpm,depsout,loop,dEmin,dEmax,M0,dE,dX,M,dres,info) ! iterative
