  subroutine dfeast_scsrevitSVD(UPLO,nrow,mcol,sa,isa,jsa,fpm,epsout,loop,Emin,Emax,M0,E,X,mode,res,info)
    !  Purpose 
    !  =======
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    double precision,dimension(min(nrow,mcol),*):: X
    integer :: mode
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    double precision,dimension(min(nrow,mcol)) :: sb ! identity
    integer,dimension(min(nrow,mcol)+1) :: isb
    integer,dimension(min(nrow,mcol)) :: jsb
    integer :: i,n
    ...
    call dfeast_scsrgvxitSVD2(UPLO,nrow,mcol,sa,isa,jsa,sb,isb,jsb,fpm,epsout,loop,Emin,Emax,M0,E,X,mode,res,info,Zne,Wne)

  end subroutine dfeast_scsrevitSVD


subroutine dfeast_scsrgvxitSVD(UPLO,nrow,mcol,sa,isa,jsa,sb,isb,jsb,fpm,epsout,loop,Emin,Emax,M0,E,X,mode,res,info,Zne,Wne)
    !  Purpose 
    !  =======
    !  FEAST SPARSE INTERFACE
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    double precision, dimension(:,:),pointer ::work,Aq,Sq,work2
...
!!!!!for pardiso
    ...
    if(fpm(11) .ne. 3) then
        print *,'Error,: must use fpm(11)=3 for minres, nothing else implemented yet'
        stop
    end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if ((UPLO=='F').or.(UPLO=='f')) then !!! full csr to upper-csr
       call wallocate_1i(sisa,nrow+1,infoloc)
       call wallocate_1i(sisb,nrow+1,infoloc)
       ...
       opt=2
       call dcsr2csr_up(opt,nrow,sa,isa,jsa,ssa,sisa,sjsa)
       call dcsr2csr_up(opt,nrow,sb,isb,jsb,ssb,sisb,sjsb)
...
!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    elseif ((UPLO=='L').or.(UPLO=='l')) then !!! lower-csr to upper-csr
       ...
       call dcsr_transpose(nrow,sa,isa,jsa,ssa,sisa,sjsa)
       call dcsr_transpose(nrow,sb,isb,jsb,ssb,sisb,sjsb)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    call wallocate_2d(work2,m,M0,infoloc)
...
!!!!!!!!!!!!!!!!! Set up for Az matrix
    ...
    call wallocate_1i(isaz,nrow+1,infoloc)
    ...
    opt=2
    if ((UPLO=='U').or.(UPLO=='u')) then
       call zdaddcsr(nrow,opt,ONEC,sa,isa,jsa,ONEC,sb,isb,jsb,saz,isaz,jsaz) !! get jsaz
    else
       call zdaddcsr(nrow,opt,ONEC,ssa,sisa,sjsa,ONEC,ssb,sisb,sjsb,saz,isaz,jsaz) !! get jsaz
    end if
!!!!!!!!!!!!!!!
    ...
    do while (ijob/=0)
        ...
        if(fpm(11)==0) then
              ...
              if(fpm(11)>0) then
                ...
                do i=1,blockits
                    ...
                    if(fpm(11)==3) call zminresSVD(UPLO,nrow,mcol,zsa,isa,jsa,ze2,workc(1,1+(i-1)*fpm(53)),ztempmat(1,1+(i-1)*fpm(53)),lintargeterror,fpm(51),linloops)    
       ...
       case(30) !! perform multiplication A*x(1:N,fpm(24):fpm(24)+fpm(25)-1) result in work(1:N,fpm(24)+fpm(25)-1)
          ...
          !    call mkl_dcsrmm('N', n, m0, n, 0.0d0, matdescra, ssa, sjsa, sisa, sisa(2), X, n, 0.0d0, work, n)
            ...
            if(nrow>mcol) then !A'A
                call mkl_dcsrmm('N', nrow, m0, mcol, 1.0d0, matdescra, sa, jsa, isa, isa(2), X, n, 0.0d0, work2, m)
                call mkl_dcsrmm('C', nrow, m0, mcol, 1.0d0, matdescra, sa, jsa, isa, isa(2), work2, m, 0.0d0, work, n)
            else !AA'
                call mkl_dcsrmm('C', nrow, m0, mcol, 1.0d0, matdescra, sa, jsa, isa, isa(2), X, n, 0.0d0, work2, m)
                call mkl_dcsrmm('N', nrow, m0, mcol, 1.0d0, matdescra, sa, jsa, isa, isa(2), work2, m, 0.0d0, work, n)
            end if
...
!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  end subroutine dfeast_scsrgvxitSVD





subroutine dfeast_scsrgvxitSVD2(UPLO,nrow,mcol,sa,isa,jsa,sb,isb,jsb,fpm,epsout,loop,Emin,Emax,M0,E,X,mode,res,info,Zne,Wne)
    !  Purpose 
    !  =======
    !  FEAST SPARSE INTERFACE
    !  Solve the generalized Ax=eBx eigenvalue problem
...
!!!!!!!!!!!!!!!!! Set up for Az matrix
    ...
    opt=1
    if ((UPLO=='U').or.(UPLO=='u')) then
       !call zdaddcsr(nrow,opt,ONEC,sa,isa,jsa,ONEC,sb,isb,jsb,saz,isaz,jsaz) !! get isaz
    else
       !call zdaddcsr(nrow,opt,ONEC,ssa,sisa,sjsa,ONEC,ssb,sisb,sjsb,saz,isaz,jsaz) !! get isaz
    end if
       ...
       case(10) !! factorize (zeB-A)
          ...
          if (fpm(11)==0) then
              ...
              if (fact) then
                 opt=3
                 if ((UPLO=='U').or.(UPLO=='u')) then
                    !call zdaddcsr(nrow,opt,-ONEC,sa,isa,jsa,Ze,sb,isb,jsb,saz(1,id),isaz,jsaz) !! get saz
                 else
                    !call zdaddcsr(nrow,opt,-ONEC,ssa,sisa,sjsa,Ze,ssb,sisb,sjsb,saz(1,id),isaz,jsaz) !! get saz
                 end if
                 if (PHASE==11) then
                    PHASE=12
                 else
...
!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  end subroutine dfeast_scsrgvxitSVD2
