...
!     %--------------%
      ...
      double precision,allocatable,dimension(:) :: workd,resid,workl,ax
...
!        | has been exceeded.                          |
!        %---------------------------------------------%
           call dsaupd ( ido, bmat, n, which, nev, tol, resid,ncv, v, n, iparam, ipntr, workd, workl,lworkl, info )
...
!c         print*,'MAIN LOOP MULTIPLICATION'    
            call mkl_dcsrmm('N', N, 1, N, 1.0d0, matdescra, dsa,jsa,isa,isa(2), workd(ipntr(1)), N, 0.0d0, workd(ipntr(2)), n) 
...
!             n, iparam, ipntr, workd, workl, lworkl, rwork, &
 !            ierr)
   call dseupd ( rvec, 'A', select, d, v, n, sigma, bmat, n, which, nev, tol, resid, ncv, v, n, iparam, ipntr, workd, workl, lworkl, ierr )
...
!c     &         ), sum(ax)                     
               call mkl_dcsrmm('N', N, 1, N, 1.0d0, matdescra, dsa, jsa, isa, isa(2), v(1,j), N, 0.0d0, ax, N) 
!c               print*,"MULTIPLICATION TEST AFTER=",sum(dsa),sum(v(1:N,j)
!c     &         ), sum(ax)                
