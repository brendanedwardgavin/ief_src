...
linepsmult=1d-2 !go this far below eigenvalue residual for linear systems
maxeig=4
    ...
    select case (ijob)
        ...
        case (11) !solve linear system Az*Qz=zwork for Qz, put Qz in zwork 
            ...
            do while (linState(1) .ne. 0)
                ...
                call zfeast_gmrespre(linjob,linState,zwork,xsol,V,Av,Ax,ze2,n,m0,maxeig,lineps,linIterations,kdim,linwork1,linwork2,zwork2,times_gm)
                linitout=linState(2) 
            ...
            !multiply by preconditioner inverse to get solution:
            ! sdfasflknasldfasdf
        ...
        case(21)
            ...
            do while (linState(1) .ne. 0)
                if(meas_acc) zt1=zwork
                !call system_clock(count=c1)
                call zfeast_gmrespre(linjob,linState,zwork,xsol,V,Av,Ax,conjg(ze2),n,m0,maxeig,lineps,linIterations,kdim,linwork1,linwork2,zwork2,times_gm)
                !call system_clock(count=c2)
                ...
                if(linjob==40) then !preconditioner only
                    linwork2=linwork1
                end if
...
open(unit=10,file='../../results/residualsout.dat',status='REPLACE')
do i=0,loop
...
write(m0str,"(I5)") m0
open(unit=10,file='../../results/rhsresidualsout.dat',status='REPLACE')
open(unit=11,file='../../results/linitsout.dat',status='REPLACE')
do i=0,loop
...
open(unit=10,file='../../results/contourpoints.dat',status='REPLACE')
do i=1,fpm(2)
    write(10,*) i,cplist(i)
end do
close(10)
...
subroutine tuned_preconditioner(n,m,X,Y,invPY) !return invPY=inv(P)*Y
    integer :: n,m
    complex (kind=kind(0.0d0)), dimension(n,*) :: X,Y,invPY

    complex (kind=kind(0.0d0)), dimension(:,:),allocatable :: xtx,ztemp
    ...
    allocate(work(lwork))
    allocate(ipiv(n))
    allocate(xtx(m,m),ztemp(n,m))

    call zgemm('T','N',m,n,n,(1.0d0,0.0d0),X(1,1),n,X(1,1),n,(0.0d0,0.0d0),xtx,m)
    do i=1,m
        xtx(i,i)=xtx(i,i)+(1.0d0,0.0d0)
    end do

    call zgetrf(m,m,xtx,m,ipiv,info)
    call zgetri(m,xtx,m,ipiv,work,lwork,info)

    !ztemp=X'*Y
    call zgemm('T','N',m,n,n,(1.0d0,0.0d0),X(1,1),n,Y(1,1),n,(0.0d0,0.0d0),ztemp(1,1),m)
    !invPY=xtx*ztemp

    !ztemp=X*invPY

    invPY=Y-ztemp
end subroutine tuned_preconditioner
