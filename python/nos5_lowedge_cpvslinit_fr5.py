...
for linits in linitss:
    outputtimes = outputtimes+","+str(linits)
    outputfres = outputfres+","+str(linits)
...
for cp in cps:
    ...
    for linits in linitss:
        ...
        outfolder="../experiments/nos5_lowedge_cpvslinit_fr5"
        ...
        basename="nos5_cp"+str(cp)+"_m"+str(m0)+"_int"+str(int)+"_e"+str(ellipse)+"_k"+str(linkspace)+"_r"+str(linits)+"_lineps"+str(lineps)+"_l"+str(maxloop)+"_feps"+str(feps)
        ...
        os.system("./time_dddriver "+projectname+" > "+outfolder+"/outfiles/"+basename+".out")
...
ff.open(outfolder+"/floopmatrix.dat")
ff.write(outputfres)
ff.close()
